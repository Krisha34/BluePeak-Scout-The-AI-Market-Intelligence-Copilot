...
export default function CompetitorsPage() {
  ...
  const handleAddCompetitor = async () => {
    ...
    try {
      ...
      setNewCompetitor({ name: '', website: '', industry: '', description: '', founded_year: undefined, headquarters: '', employee_count: undefined })
      loadCompetitors()
    } catch (error) {
  ...
  const handleAnalyze = async (id: string, name: string) => {
    try {
      toast.loading(`Analyzing ${name}...`, { id: 'analyze' })
      await apiClient.analyzeCompetitor(id)
      toast.success('Analysis complete!', { id: 'analyze' })
      loadCompetitors()
    } catch (error) {
      console.error('Error analyzing competitor:', error)
      toast.error('Failed to analyze competitor', { id: 'analyze' })
    }
  }
    ...
    <div className="p-4 md:p-8 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        <div>
          ...
          <p className="text-gray-600 mt-2">Monitoring {competitors.length} competitors • Last updated: {new Date().toLocaleTimeString()}</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="mt-4">
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className="font-medium">{((competitors.filter(c => c.status === 'active').length / competitors.length) * 100).toFixed(0)}%</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                className="bg-green-600 h-2 rounded-full"
                style={{ width: `${(competitors.filter(c => c.status === 'active').length / competitors.length) * 100}%` }}
              />
            </div>
          </div>
        </div>
            ...
            <div className="space-y-2 mb-4">
              {competitor.website && (
                <div className="flex items-center gap-2 text-sm text-gray-600">
                  ...
                  <a href={competitor.website} target="_blank" rel="noopener noreferrer" className="hover:text-blue-600 truncate">
                    {competitor.website}
                  </a>
                </div>
              )}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewCompetitor({ ...newCompetitor, founded_year: parseInt(e.target.value) })}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                      placeholder="2020"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewCompetitor({ ...newCompetitor, employee_count: parseInt(e.target.value) })}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                      placeholder="100"
                    />
                  </div>
